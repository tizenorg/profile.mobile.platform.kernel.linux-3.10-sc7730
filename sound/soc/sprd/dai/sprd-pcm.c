			      ...
			      struct snd_pcm_hw_params *params)
{
	...
	if(params_periods(params)) {
	    *(dma_desc[0]) = *(dma_desc[0]-1);
	    if(used_chan_count == 2) {
		*(dma_desc[1]) = *(dma_desc[1] - 1);
	    }
	}
	...
	sci_dma_config((u32) (rtd->uid_cid_map[0]),
		       ...
		       (params_periods(params)==1)?2:params_periods(params), &dma_reg_addr[0]);
	...
	if (used_chan_count > 1) {
		...
		sci_dma_config((u32) (rtd->uid_cid_map[1]),
			       (struct sci_dma_cfg *)(rtd->dma_cfg_array +
						      runtime->hw.periods_max),
			       (params_periods(params)== 1)?2:params_periods(params), &dma_reg_addr[1]);
		if (!(params->flags & SNDRV_PCM_HW_PARAMS_NO_PERIOD_WAKEUP)) {
			sp_asoc_pr_info("Register IRQ for DMA Chan ID %d\n",
					rtd->uid_cid_map[1]);
			ret =
