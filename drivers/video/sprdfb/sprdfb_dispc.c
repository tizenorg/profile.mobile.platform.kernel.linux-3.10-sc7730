...
static void dispc_dump_reg(const char *str)
{
	int32_t i;

	printk("%s:%s\n", __func__, str);
	for(i = 0; i < 256; i += 16)
		printk("%x: 0x%08x 0x%08x 0x%08x 0x%08x\n", i, dispc_read(i),
			dispc_read(i + 4), dispc_read(i + 8), dispc_read(i + 12));
	printk("**************************************\n");
}
...
#ifdef CONFIG_OF
	dev->irq = irq_of_parse_and_map(dev->of_dev->of_node, 0);
#else
	dev->irq = IRQ_DISPC_INT;
#endif
	printk("sprdfb: dispc irq_num = %d\n", dev->irq);
	//ret = request_irq(IRQ_DISPC_INT, dispc_isr, IRQF_DISABLED, "DISPC", &dispc_ctx);
	ret = request_irq(dev->irq, dispc_isr, IRQF_DISABLED | IRQF_SHARED, "DISPC", &dispc_ctx);
	if (ret) {
...
//	unsigned long irqflags;
	...
	pr_info("%s:type[%d]refresh[%d]count[%d]done\n", __func__,
...
static int sprdfb_dispc_clk_enable(struct sprdfb_dispc_context *dispc_ctx_ptr, SPRDFB_DYNAMIC_CLK_SWITCH_E clock_switch_type)
{
	...
	pr_info("%s:type[%d]refresh[%d]count[%d]ret[%d]\n", __func__,
...
#ifdef CONFIG_FB_ESD_SUPPORT
		if (!dev->ESD_work_start) {
			printk(KERN_INFO "sprdfb: schedule ESD work queue!\n");
			schedule_delayed_work(&dev->ESD_work, msecs_to_jiffies(dev->ESD_timeout_val));
			dev->ESD_work_start = true;
		}
#endif
		...
		overlay_rect *rect, overlay_endian *endian, bool rb_switch)
{
	...
	reg_value = (y_endian << 8 | type << 4 | BIT(2) | BIT(17));
	if(rb_switch)
		reg_value |= BIT(15);
...
static int32_t sprdfb_dispc_enable_overlay(struct sprdfb_device *dev, struct overlay_info *info)
{
	...
	if (NULL == info || ((info->layer_index != SPRD_LAYER_IMG) &&
		...
		pr_err("sprdfb: [%s]: Invalid parameter: info[%p], layer index[%d]\n",
				__func__, info, info ? info->layer_index : -1);
...
static int32_t sprdfb_dispc_display_overlay(struct sprdfb_device *dev, struct overlay_display* setting)
{
...
#ifdef CONFIG_DRM_SPRD
#ifdef CONFIG_SPRDFB_USE_GEM_INDEX
		phys_addr = *((unsigned long *)sprd_drm_gem_get_obj_addr(
			setting->img_handle.handle, setting->img_handle.index));
#else
		phys_addr = *((unsigned long *)sprd_drm_gem_get_obj_addr(
	...
	if(SPRD_OVERLAY_STATUS_ON == dispc_ctx.overlay_state
		|| SPRD_OVERLAY_STATUS_STARTED == dispc_ctx.overlay_state){
...
ERROR_DISPLAY_OVERLAY:
	...
	if (dev->dbg_cnt)
		pr_info("%s:layer[%d]osd[0x%x]img[0x%x]c[%d]done\n",
			"disp_ovl", setting->layer_index, (unsigned int)osd_buffer,
			(unsigned int)img_buffer, --dev->dbg_cnt);
...
void dispc_update_mode(bool enable)
{
	pr_info("%s[%d]\n", __func__, enable);
	...
	if (enable) {
		dispc_set_bits(BIT(4), DISPC_DPI_CTRL);
		dispc_write(0, DISPC_INT_EN);
		disable_irq(dispc_ctx.dev->irq);
	} else {
		enable_irq(dispc_ctx.dev->irq);
		dispc_clear_bits(BIT(4), DISPC_DPI_CTRL);
		dispc_write(DISPC_INT_UPDATE_DONE_MASK | DISPC_INT_HWVSYNC |
					DISPC_INT_ERR_MASK, DISPC_INT_EN);
	}
