...
volatile struct sec_debug_regs_access *sec_debug_last_regs_access;
...
static void simulate_panic(void)
{
	pr_emerg("Simulating panic\n");
	panic("panic simulation");
	/* if we hit here, test had failed */
	pr_emerg("Simulation of panic failed\n");
}
...
static int force_error(const char *val, struct kernel_param *kp)
{
	...
	if (!strncmp(val, "appdogbark", 10)) {
		...
		simulate_bus_hang();
	} else if (!strncmp(val, "panic", 5)) {
		pr_emerg("Generating a Panic!\n");
		simulate_panic();
	} else {
				   ...
				   unsigned long l, void *buf)
{
	...
	else if (!strcmp(buf, "power_key_long_press"))
