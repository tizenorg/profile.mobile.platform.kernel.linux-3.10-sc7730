...
unsigned int cpu_load_history_num = CPU_LOAD_HISTORY_NUM;
...
void store_killed_task(struct task_struct *tsk)
{
	...
	strncpy(saved_task_info[cnt].comm, tsk->comm, TASK_COMM_LEN - 1);
}
	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	size_for_copy = wrapper_for_debug_fs(buffer, count, ppos
									, cpuidle_w_aftr_jig_check_en_read_sub);
...
void load_anlyzer_data_memcpy(void)
{
	...
	memcpy(input_rec_history_view, input_rec_history
				, INPUT_REC_HISTORY_SIZE);
	b_input_load_data = 0;
#endif
}
