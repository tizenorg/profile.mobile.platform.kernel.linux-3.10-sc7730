...
int input_rec_store_input_dev_info(void)
{
	...
	for (i=0; i<cnt; i++) {
		pr_info("Current [%d]dev=%p, name=%s\n", i
				, input_dev_info_current[i].dev, input_dev_info_current[i].name);
	}
...
int input_rec_load_data(void)
{
	...
	if (input_dev_info_saved_num >= MAX_INPUT_DEVICES) {
		pr_err("input_dev_info_saved_num: %d is greater than array index range\n", input_dev_info_saved_num);
		return -1;
	}
...
#endif
#if defined(ROTARY_BOOSTER)
	if (strcmp(dev->name, rotary_booster_name) == 0) {
		switch(type) {
