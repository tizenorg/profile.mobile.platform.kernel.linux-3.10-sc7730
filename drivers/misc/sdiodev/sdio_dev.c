...
#endif
	atomic_t              in_sdio_suspend;
	atomic_t              refcnt;
}mgr_suspend_t;

#define SDIO_OS_LOCK()  do{\
	if(1 == atomic_read(&g_mgr_suspend.in_sdio_suspend)) \
	{\
...
static void sdio_early_suspend(struct early_suspend *es)
{
	...
	while(0 != atomic_read(&(g_mgr_suspend.refcnt)) )
	{
		...
		{
			SDIOTRAN_ERR("[%s][timeout 3000ms][EEROR]", __func__);
			return;
		}
	}
	SDIOTRAN_ERR("[%s]ok", __func__);
	return;
}
...
void gpio_timer_handler(unsigned long data)
{
	if(gpio_get_value(sdio_data->wake_ack))
	{
		...
		mod_timer(&(sleep_para.gpio_timer),\
			jiffies + msecs_to_jiffies(200));//   250<300<2000-1500
...
unsigned int marlin_sdio_req_cnt = 0;
unsigned int fdl_sdio_req_cnt = 0;
unsigned int sch_sdio_req_cnt = 0;
 ...
 //bt_state = get_bt_state();
    ...
    if(0 == sleep_para.gpio_opt_tag)
	{
		if(gpio_get_value(sdio_data->wake_ack))  //bt ack 250ms
		{
			...
			SDIOTRAN_ERR("%d-high,user_id-%d", sdio_data->wake_ack,user_id);
			return -2;
	}
		...
		if(0 == atomic_read(&(set_marlin_cmplete)))  //last wakeup don't complete
	{
		...
		SDIOTRAN_ERR("S%d-1", sdio_data->wake_out );
		...
		if((0x1 == user_id) ||(0x3 == user_id))
		{
			...
			if (ret == 0){
				...
				SDIOTRAN_ERR("marlin chn %d ack timeout, user_id-%d, sdio_cnt %d,fdl_cnt %d,sch_cnt %d!!!",chn,user_id,marlin_sdio_req_cnt,fdl_sdio_req_cnt,sch_sdio_req_cnt);
...
int set_marlin_sleep(uint32 chn,uint32 user_id)
{
	...
	if( atomic_read(&gpioreq_need_pulldown) )//if(sleep_para.gpioreq_need_pulldown)
	{
		...
		SDIOTRAN_ERR("gpio%d-0,user_id-%d", sdio_data->wake_out, user_id);
	}
#endif
	return 0;
}
...
void invalid_recv_flush(int chn)
{
	int len ;
	char* pbuff = NULL;
...
static void marlin_workq(void)
{
	...
	switch(read_chn)
	{
		...
		case WIFI_CHN_9:
			...
			break;
		default:
			SDIOTRAN_ERR("no handler for chn %d",read_chn);
			invalid_recv_flush(read_chn);
	}
...
static irqreturn_t marlinwake_irq_handler(int irq, void * para)
{
	...
	SDIOTRAN_ERR("%d-%d",sdio_data->wake_ack, gpio_wake_status );
	...
	if(gpio_wake_status)
	{
		if( atomic_read(&gpioreq_need_pulldown) )//if(sleep_para.gpioreq_need_pulldown)
		{
			...
			{
				SDIOTRAN_ERR("sdio_w_flag:%d", sdio_w_flag);
...
static int marlin_sdio_resume(struct device *dev)
{
	...
	mod_timer(&(sleep_para.gpio_timer),jiffies + msecs_to_jiffies(350));
	/***************/
	smp_mb();
	SDIOTRAN_ERR("[%s]ok", __func__);
	return 0;
...
static int _marlin_gpio_init(void)
{
	int ret;
	...
	ret = gpio_request(sdio_data->wake_out, "sdio_wake_out");
	if (ret<0) {
		SDIOTRAN_ERR("GPIO_AP_TO_MARLIN = %d gpio request failed",\
			sdio_data->wake_out);
			return ret;
	}
	...
	ret = gpio_request(sdio_data->data_irq, "sdio_data_irq");
	if (ret<0) {
		SDIOTRAN_ERR("GPIO_MARLIN_TO_AP = %d gpio request failed",\
			sdio_data->data_irq);
			return ret;
	}
	...
	ret = gpio_request(sdio_data->wake_ack, "sdio_wake_ack");
	if (ret<0) {
		SDIOTRAN_ERR("GPIO_MARLIN_WAKE = %d gpio request failed",\
			sdio_data->wake_ack);
			return ret;
	}
	...
	ret = gpio_request(sdio_data->io_ready, "sdio_io_ready");
	if (ret<0)
	{
		SDIOTRAN_ERR("GPIO_MARLIN_SDIO_READY = %d gpio request failed",\
			sdio_data->io_ready);
		return ret;
	}
