...
#if defined(CONFIG_MACH_COREPRIMEVE3G) || defined(CONFIG_MACH_J23G)
...
LOCAL int sensor_k_remove(struct platform_device *dev)
{
	...
	get_gpio_id_ex(p_mod->of_node, GPIO_CAMIOVDD, &gpio_id, 0);
	gpio_free(gpio_id);
	get_gpio_id_ex(p_mod->of_node, GPIO_CAMIOVDD, &gpio_id, 1);
	gpio_free(gpio_id);
#endif
...
LOCAL int sensor_k_suspend(struct platform_device *dev)
{
	...
	for (i = 0; i < 2; i++) {
		...
		gpio_set_value(gpio_tab.reset, 1);
	}
...
#else // CONFIG_MACH_COREPRIMEVE3G || CONFIG_MACH_J23G
