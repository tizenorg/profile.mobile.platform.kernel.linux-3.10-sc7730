...
void ist30xx_special_cmd(struct ist30xx_data *data, int cmd)
{
	...
	tsp_warn("Not support special cmd: 0x%02X\n", cmd);
}
#endif
...
#define DEEP_SLEEP_MASK		(0x00080000)
...
static int ist30xx_get_sleep_monitor_cb(void* priv, unsigned int *raw_val, int check_level, int caller_type);

static struct sleep_monitor_ops  ist30xx_sleep_monitor_ops = {
	 .read_cb_func =  ist30xx_get_sleep_monitor_cb,
};
...
out:
	*raw_val = ((state & STATE_MASK) << STATE_BIT) |\
	...
	tsp_info("%s: raw_val[0x%08x], check_level[%d], state[%d], pretty[%d]\n",
...
static int ist30xx_set_input_device(struct ist30xx_data *data)
{
	...
	input_set_abs_params(data->input_dev, ABS_MT_POSITION_Y,
			0, data->max_y - 1, 0, 0);
	input_set_abs_params(data->input_dev, ABS_X, 0, data->max_x - 1, 0, 0);
	input_set_abs_params(data->input_dev, ABS_Y, 0, data->max_y - 1, 0, 0);

	input_set_abs_params(data->input_dev, ABS_MT_TOUCH_MAJOR, 0, IST30XX_MAX_W, 0, 0);
	input_set_abs_params(data->input_dev, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
	input_set_abs_params(data->input_dev, ABS_MT_TRACKING_ID, 0, 3, 0, 0);
	...
	if (ret) {
		tsp_err("%s: failed input register device:%d\n", __func__, ret);
	} else
		...
		const struct i2c_device_id *id)
{
...
#ifdef CONFIG_SLEEP_MONITOR
	sleep_monitor_register_ops(data, &ist30xx_sleep_monitor_ops,
			SLEEP_MONITOR_TSP);
#endif
	...
	if (in_calibration()) {
		tsp_info("%s: power_off for cal_mod\n", __func__);
		data->touch_stopped = true;
		ist30xx_suspend(&data->client->dev);
	}
