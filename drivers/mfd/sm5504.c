...
#ifdef CONFIG_MUIC_SUPPORT_FACTORY
struct sm5504_chip *chip_g = NULL;
#ifdef CONFIG_MUIC_FACTORY_EVENT
 ...
 */
static ATOMIC_NOTIFIER_HEAD(muic_notifier_list);

int register_muic_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_register(&muic_notifier_list, nb);
}

int unregister_muic_notifier(struct notifier_block *nb)
{
	return atomic_notifier_chain_unregister(&muic_notifier_list, nb);
}
...
static void sm5504_preprocess_status(sm5504_chip_t *chip)
{
	...
	chip->curr_status.ovp_status =
		...
		((chip->curr_status.irq_flags[1] & 0x80) == 0x00)) ||
					...
					unsigned int new_status)
{
	...
	if(!chip->curr_status.ovp_status){
		...
		if ((chip->curr_status.cable_type ==
			MUIC_SM5504_CABLE_TYPE_JIG_UART_OFF_WITH_VBUS) ||
			(chip->curr_status.cable_type ==
			MUIC_SM5504_CABLE_TYPE_JIG_UART_ON_WITH_VBUS)) {
				usb_mode = get_usb_mode();
				if ((usb_mode == NOTIFY_TEST_MODE) ||
					(usb_mode == NOTIFY_HOST_MODE)) {
						pr_info("%s:skip charging cb. factory otg test\n", __func__);
				} else {
				      ...
				      unsigned int new_status)
{
	struct muic_notifier_param param = {
		.vbus_status = chip->curr_status.vbus_status,
...
static void sm5504_uart_attach_handler(struct sm5504_chip *chip,
				       ...
				       *handler, unsigned int old_status,
				       unsigned int new_status)
{
#if defined(CONFIG_MUIC_SUPPORT_RUSTPROOF) && !defined(CONFIG_MUIC_SUPPORT_FACTORY) && !defined(CONFIG_MACH_PIKEAYOUNG2DTV)
				      ...
				      unsigned int new_status)
{
...
#if defined (CONFIG_MUIC_SUPPORT_RUSTPROOF)
	{
		...
		/*battery detection*/
		val = sec_vf_adc_check();

		if ( (is_usrbin == 1) &&
			((type==JIG_UART_BOOT_OFF) || (type==JIG_UART_BOOT_ON)) ) {
			if (val == 1) {
				sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1, 0x00);
				sm5504_clr_bits(chip, SM5504_REG_CONTROL, 1 << 2);//auto switching -> manual switching
				is_rustproof = 1;
				chip->rustproof = 1;
				RTINFO("Open JIG UART PATH in case of battery on USR binary\n");
			} else { // no batt
				sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1, 0x6C);
				sm5504_clr_bits(chip, SM5504_REG_CONTROL, 1 << 2);//auto switching -> manual switching
				is_rustproof = 1;
				chip->rustproof = 0;
				RTINFO("Connect JIG UART PATH in case of no battery on USR binary\n");
			}
		} else {
				      ...
				      unsigned int new_status)
{
    ...
    if (is_rustproof) {
        sm5504_reg_write(chip, SM5504_REG_MANUAL_SW1, 0x00);
        sm5504_set_bits(chip, SM5504_REG_CONTROL, 1 << 2); //manual switching -> auto switching
        is_rustproof = 0;
		chip->rustproof = 1;
        RTINFO("Open JIG UART PATH by detaching\n");
    } else {
        if (chip->pdata->jig_callback)
...
static void sm5504_irq_work(sm5504_chip_t *chip)
{
	...
	struct muic_notifier_param param = { .vbus_status = 0, .cable_type = 0};
...
/* for printint out registers -- end*/
	...
	reg22 = sm5504_reg_read(chip, 0x22);
	reg3A = sm5504_reg_read(chip, 0x3A);
	RTINFO("reg22 = 0x%x reg3A = 0x%x\n",reg22,reg3A);
	...
	if (chip->curr_status.vbus_status != chip->prev_status.vbus_status) {
		RTINFO("SM5504 set the vbus noti to usb \n");

		param.vbus_status = chip->curr_status.vbus_status;
		param.cable_type = chip->curr_status.cable_type;

		atomic_notifier_call_chain(&muic_notifier_list,
				MUIC_VBUS_NOTI, &param);
	} else {
		RTINFO("no vbus \n");
	}
...
#ifdef CONFIG_MUIC_SUPPORT_RUSTPROOF
	{/*check usr binary*/
		int val_sm5504_reg13;
		int val_sm5504_reg2;

		val_sm5504_reg13 = sm5504_reg_read(chip, SM5504_REG_MANUAL_SW1);
		if (val_sm5504_reg13 == 0x0C)
		{
			is_usrbin = 1;
			val_sm5504_reg2 = sm5504_reg_read(chip, SM5504_REG_CONTROL);
			if (!(val_sm5504_reg2 & 0x04))
				sm5504_set_bits(chip, SM5504_REG_CONTROL, 1 << 2); //manual switching -> auto switching
		}
		RTINFO("Kernel Binary: %s(0x%x)\n", is_usrbin ? "USR" : "ENG", val_sm5504_reg13);
	}
#endif
	/* Dummy read */
	sm5504_reg_read(chip, SM5504_REG_INT_FLAG1);
	sm5504_reg_read(chip, SM5504_REG_INT_FLAG2);
#if 0
...
static void sm5504_init_regs(sm5504_chip_t *chip)
{
	...
	//sm5504_reg_write(chip, SM5504_REG_RESET, 0x01);
			...
			struct device_attribute *attr, char *buf)
{
	...
	if(chip_g)
		adc_value = sm5504_reg_read(chip_g, chip_g->adc_reg_addr);
	...
	RTERR("%s line %d : adc_value:0X%X\n", __FUNCTION__,__LINE__,adc_value);
...
#ifdef CONFIG_MUIC_SUPPORT_RUSTPROOF
static ssize_t rustproof_store_attrs(
					struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d\n", &x) == 1) {
		chip_g->rustproof = x;
		if (x == 1) {
			sm5504_reg_write(chip_g, SM5504_REG_MANUAL_SW1, 0x00);
			sm5504_set_bits(chip_g, SM5504_REG_CONTROL, 1 << 2); //manual switching -> auto switching
			RTINFO("activate rustproof mode\n");
		} else if (x == 0) {
			sm5504_reg_write(chip_g, SM5504_REG_MANUAL_SW1, 0x6C);
			sm5504_clr_bits(chip_g, SM5504_REG_CONTROL, 1 << 2);//auto switching -> manual switching
			RTINFO("deactivate rustproof mode\n");
		} else {
			RTINFO("%s: unknown state\n", __func__);
			return -EINVAL;
		}
		return count;
	}
...
static ssize_t rustproof_show_attrs(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	int i = 0;
...
#ifdef CONFIG_MUIC_SUPPORT_RUSTPROOF
static DEVICE_ATTR(rustproof, S_IRUGO|S_IWUGO, rustproof_show_attrs, rustproof_store_attrs);
#endif
...
#ifdef CONFIG_SWITCH
	pdata->init_callback = switch_data.init_cb;
	pdata->dock_callback = switch_data.dock_cb;
	pdata->usb_callback  = switch_data.usb_cb;
	...
	if (chip) {
	   free_irq(chip->irq, chip);
		...
		if (chip->wq)
			destroy_workqueue(chip->wq);
		wake_lock_destroy(&chip->muic_wake_lock);
#if defined(CONFIG_MUIC_SUPPORT_FACTORY) &&  defined(CONFIG_SAMSUNG_MUIC)
       	sysfs_remove_group(&chip->switch_dev->kobj, &sm5504_muic_group);
#elfi !defined(CONFIG_USB_SPRD_SWITCH)
	sysfs_remove_group(&switch_devices->kobj, &sm5504_muic_group);
#endif
		kfree(chip);
	}
	return 0;
