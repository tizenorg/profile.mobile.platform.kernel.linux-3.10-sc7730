...
#else
	/* In latest kernel, smp_mb__after_clear_bit is replaced with
	  * smp_mb__after_atomic. So, if kernel is migrated to latest,
	  * then below code should be enabled
	  */
...
static void hci_auth_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!conn)
        ...
        if (ev->status == 0x06) {
                ...
                hci_send_cmd(conn->hdev, HCI_OP_AUTH_REQUESTED, sizeof(cp),
							 &cp);
                goto unlock;
        }
#endif
				    ...
				    struct sk_buff *skb)
{
	...
	if (!ev->status && !test_bit(HCI_CONN_MGMT_CONNECTED, &conn->flags)) {
		struct hci_cp_change_conn_ptype cp;
		memset(&cp, 0, sizeof(cp));
		cp.pkt_type = (HCI_DM1|HCI_DH1);
		if (conn->features[0][0] & LMP_3SLOT)
			cp.pkt_type |= (HCI_DM3|HCI_DH3);

		if (conn->features[0][0] & LMP_5SLOT)
			cp.pkt_type |= (HCI_DM5|HCI_DH5);

		if (!(conn->features[0][3] & LMP_EDR_ACL_2M)) {
			cp.pkt_type |= (HCI_2DH1|HCI_2DH3|HCI_2DH5);
		} else {
			if (!(conn->features[0][4] & LMP_EDR_3SLOT))
				cp.pkt_type |= HCI_2DH3;

			if (!(conn->features[0][5] & LMP_EDR_5SLOT))
				cp.pkt_type |= HCI_2DH5;
		}

		if (!(conn->features[0][3] & LMP_EDR_ACL_3M)) {
			cp.pkt_type |= (HCI_3DH1|HCI_3DH3|HCI_3DH5);
		} else {
			if (!(conn->features[0][4] & LMP_EDR_3SLOT))
				cp.pkt_type |= HCI_3DH3;

			if (!(conn->features[0][5] & LMP_EDR_5SLOT))
				cp.pkt_type |= HCI_3DH5;
		}
